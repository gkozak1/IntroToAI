...
const MODEL_PROFILES = {
  ...
  lfm: {
    ...
    hasAttention: false,
    promptMode: 'raw',
    hiddenPrefixTokens: 1,
    explanation: '<strong>Modern mode:</strong> LFM2.5 receives the same raw passage as GPT-2 and predicts the token immediately after it. Your Temperature and Top-K settings remain unchanged for a controlled comparison.',
...
class LFMEngine {
  ...
  encode(text) {
    const passageIds = Array.from(
      this.tokenizer.encode(text === '' ? ' ' : text, { add_special_tokens: false }),
      Number
    );
    ...
    // LFM is instruction-tuned, but its causal model still performs true raw
    // completion when the passage follows its BOS token. This gives both modes
    // the same task: predict the token immediately after the visible passage.
    return passageIds[0] === bosTokenId ? passageIds : [bosTokenId, ...passageIds];
  }
...
async function analyzePrompt({ force = false } = {}) {
  ...
  try {
    ...
    // Both engines receive the trimmed passage directly. LFM2.5 prepends only
    // its required invisible beginning-of-text token.
