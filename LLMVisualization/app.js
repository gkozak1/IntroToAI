...
const MODEL_PROFILES = {
  ...
  lfm: {
    ...
    explanation: '<strong>Modern mode:</strong> LFM2.5 is instruction-tuned, so it receives a short invisible continuation instruction and your passage is prefilled as the start of its answer. New tokens still begin immediately after your final character, and the displayed Temperature, Top-K, logits, and probabilities are the exact values used for sampling.',
...
const LFM_CONTINUATION_INSTRUCTION = 'Continue the supplied passage from its exact final character. Write only the continuation: do not quote, restart, explain, summarize, or label the passage.';

function buildLfmCompletionContext(passage) {
  // LFM2.5-350M is instruction-tuned. The passage appears in the user request
  // and is then prefilled as the beginning of the assistant response. This
  // makes the next causal-model position the exact character boundary the
  // student sees, without exposing a duplicated passage in the completion UI.
  return [
    '<|startoftext|>',
    `<|im_start|>system\n${LFM_CONTINUATION_INSTRUCTION}<|im_end|>\n`,
...
class LFMEngine {
  ...
  encode(text) {
    const passage = text === '' ? ' ' : text;
    const modelContext = buildLfmCompletionContext(passage);
    const contextIds = Array.from(
      this.tokenizer.encode(modelContext, { add_special_tokens: false }),
      Number
    );
    const bosTokenId = Number(this.tokenizer.bos_token_id);
    if (!Number.isInteger(bosTokenId)) throw new Error('LFM2.5 tokenizer did not provide its required beginning-of-text token.');
    if (contextIds[0] !== bosTokenId) {
      throw new Error('LFM2.5 tokenizer did not recognize the chat template beginning-of-text token.');
    }
    return contextIds;
  }
...
async function analyzePrompt({ force = false } = {}) {
  ...
  try {
    ...
    // GPT-2 receives the passage directly. LFM2.5's encode method builds the
    // disclosed instruction context and prefills the passage in its response.
...
function updateRoundStatus() {
  const contextLength = Math.max(0, state.tokenIds.length - (currentProfile().hiddenPrefixTokens || 0));
